...
module.exports = async function handler(req, res) {
  ...
  try {
    const { name, email, company, location, total, teaser } = req.body;
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #3E4E63;">
        ...
        <div style="background: #F7F5F2; padding: 32px 36px; border-radius: 0 0 8px 8px; border: 1px solid #D8D4CD;">
          ...
          <p style="color: #3E4E63; line-height: 1.7; margin-top: 0;">Here are your Free Profit Leak Scan results for <strong>${bizName}${bizLocation}</strong>.</p>
...
${teaser}
          ...
          <div style="background: white; border-left: 4px solid #C8701A; border-radius: 0 8px 8px 0; padding: 18px 20px; margin-bottom: 20px; border-top: 1px solid #D8D4CD; border-right: 1px solid #D8D4CD; border-bottom: 1px solid #D8D4CD;">
            ...
            <p style="margin: 0 0 12px; font-size: 13px; color: #3E4E63; line-height: 1.65;">
              This scan covers 5 of 10 leak categories. The <strong>In-Depth Scan & Report</strong> covers all 10 — with your specific numbers, leaks ranked by dollar amount, and a prioritized to-do list built around your answers. Includes all 10 Fix-It Guides and all 8 Process Doc templates.
            </p>
            <a href="https://compassbizsolutions.com" style="display: inline-block; background: #C8701A; color: white; font-weight: bold; font-size: 13px; padding: 10px 20px; border-radius: 7px; text-decoration: none;">Get the In-Depth Scan — $249 →</a>
          </div>
          ...
          <div style="background: white; border-radius: 8px; padding: 18px 20px; margin-bottom: 24px; border: 1px solid #D8D4CD;">
            ...
            <a href="https://compassbizsolutions.com" style="display: inline-block; background: #3D6B9E; color: white; font-weight: bold; font-size: 13px; padding: 10px 20px; border-radius: 7px; text-decoration: none;">Get the Fix-It Bundle — $47 →</a>
          </div>
    ...
    await resend.emails.send({
      ...
      to:      email,
      subject: `Your Profit Leak Scan Results — ${bizName} — ${leakAmount} estimated`,
      html,
    });
